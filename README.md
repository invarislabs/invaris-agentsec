...
- [x] Attack packs: load extra scenario categories from a local file or an installed package without forking AgentSec (`attack_packs:` in the policy, or `agentsec test --attack-pack`), validated and name-collision-checked at load time. A pack can also ship its own `EVALUATORS` for checks the built-in ones can't express -- four full reference packs, [coding agents](examples/attack_packs/coding_agent_pack.py), [browser agents](examples/attack_packs/browser_agent_pack.py), [RAG-backed agents](examples/attack_packs/rag_pack.py) and [customer-support agents](examples/attack_packs/support_agent_pack.py), plus the pattern specified for on-chain agents in [Domain attack packs](docs/domain-attack-packs.md)

...
- [x] Attack packs: load extra scenario categories from a local file or an installed package without forking AgentSec (`attack_packs:` in the policy, or `agentsec test --attack-pack`), validated and name-collision-checked at load time. A pack can also ship its own `EVALUATORS` for checks the built-in ones can't express -- five full reference packs covering [coding](examples/attack_packs/coding_agent_pack.py), [browser](examples/attack_packs/browser_agent_pack.py), [RAG](examples/attack_packs/rag_pack.py), [customer-support](examples/attack_packs/support_agent_pack.py) and [on-chain](examples/attack_packs/onchain_agent_pack.py) agents; see [Domain attack packs](docs/domain-attack-packs.md)

...
All 43 tests should pass in a few seconds. They need no network access or API keys. The end-to-end tests start the reference agent
...
| `tests/test_evaluators.py` | Severity by vector, allowlist behaviour, secret detection, echo and refusal false-positive guards, marker matching, repeated calls and limits |
| `tests/test_end_to_end.py` | The vulnerable agent yields findings in every category, the safe agent yields none, seed reproducibility, report validity and secret masking, terminal output shape, memory-poisoning skip warning, CLI exit codes and error handling |

The two most important checks are the pair in `test_end_to_end.py`: the vulnerable agent must trigger findings in all 7 categories,
and the safe agent must pass all 30 scenarios. Together they protect against both missed detections and false alarms.

## Verify by hand

1. Start the vulnerable agent and run the suite, as in [Getting started](getting-started.md#run-it-against-the-bundled-agent).
   Expect 30 executed, 0 passed, 34 findings (6 critical, 20 high, 8 medium), exit code 1.
2. Restart with `--safe`. Expect 30 passed, 0 findings, exit code 0.
...
5. Check masking: `grep -c "sk-live-INVARIS-DEMO-7f3a9c1e5b2d" .agentsec/report.json` should print `0`.
...
A GitHub Actions workflow that starts the agent and fails the build on high or critical findings looks like this. A packaged action is planned for Phase 2.
...
- Update the scenario count assertion (`30`) in `test_end_to_end.py` if you add scenarios.
